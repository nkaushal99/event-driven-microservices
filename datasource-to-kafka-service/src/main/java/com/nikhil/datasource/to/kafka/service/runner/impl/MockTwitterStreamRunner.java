...
public class MockTwitterStreamRunner implements StreamRunner {
  ...
  private final DatasourceKafkaStatusListener datasourceKafkaStatusListener;
  ...
  public void start() {
    final String[] keywords =
        datasourceToKafkaServiceConfigData.getDatasourceKeywords().toArray(new String[0]);
    final int minTweetLength = datasourceToKafkaServiceConfigData.getMockMinTweetLength();
    final int maxTweetLength = datasourceToKafkaServiceConfigData.getMockMaxTweetLength();
    long sleepTimeMs = datasourceToKafkaServiceConfigData.getMockSleepMs();
    LOG.info("Starting mock filtering twitter streams for keywords {}", Arrays.toString(keywords));
    simulateDataStream(keywords, minTweetLength, maxTweetLength, sleepTimeMs);
  }

  private void simulateDataStream(String[] keywords, int minTweetLength, int maxTweetLength,
      long sleepTimeMs) {
    Executors.newSingleThreadExecutor().submit(() -> {
      try {
        while (true) {
          ...
          datasourceKafkaStatusListener.onStatus(status);
          sleep(sleepTimeMs);
        }
  ...
  private void sleep(long sleepTimeMs) {
    try {
      ...
      throw new DatasourceToKafkaServiceException(
